...
class Prism{
    public:
        ...
        void set_eta_r(double eta_r){
            this->eta_r = eta_r;
        }

        void set_eta_g(double eta_g){
            this->eta_g = eta_g;
        }

        void set_eta_b(double eta_b){
            this->eta_b = eta_b;
        }

        void set_t1(Triangle *t1){
            this->t1 = t1;
        }

        void set_t2(Triangle *t2){
            this->t2 = t2;
        }

        void set_t3(Triangle *t3){
            this->t3 = t3;
        }

        void set_t4(Triangle *t4){
            this->t4 = t4;
        }


        struct point Refracted_dir(struct point incidentVec, struct point normal, double eta)
        {
            cout<<"incident point =>"<<incidentVec.x<<" "<<incidentVec.y<<" "<<incidentVec.z<<" "<<endl;
            cout<<"normal point =>"<<normal.x<<" "<<normal.y<<" "<<normal.z<<" "<<endl;
            double N_dot_I = dot_prod(normal,incidentVec);
            cout<<"n_dot_i"<<N_dot_I<<endl;
            ...
            {
                struct point temp;
                temp.x = incidentVec.x*eta - normal.x*(eta * N_dot_I + sqrt(k));
                temp.y = incidentVec.y*eta - normal.y*(eta * N_dot_I + sqrt(k));
                temp.z = incidentVec.z*eta - normal.z*(eta * N_dot_I + sqrt(k));
                cout<<"vector value="<<temp.x<<" "<<temp.y<<" "<<temp.z<<" "<<endl;
        ...
        struct point Get_Floor_Intersects(double eta){
            cout<<"eta: "<<eta<<endl;
            cout<<"start get_R_floor_points"<<endl;

            vector<struct point> R_floor_points;

            for(int i=0; i< spot_lights.size(); i++){


                double t_closest_object_min = 999999;
                int closest_object_index = 999999;
                struct point source_ray_position = spot_lights[i]->point_light.light_pos;
                Ray temp_ray(source_ray_position, unit_vector(spot_lights[i]->light_dir));
                double t_min_t1 = t3->get_tMIN(&temp_ray);
                double t_min_t2 = t4->get_tMIN(&temp_ray);

                ///double t_min_t1 = t3->get_t_min(Ray(source_ray_position, spotlights[i].direction.get_unit_Vector3D()));
                ///double t_min_t2 = t4->get_t_min(Ray(source_ray_position, spotlights[i].direction.get_unit_Vector3D()));

                double t_min_min = 999999;
                ...
                if(t_min_t1<999999){
                    ...
                    t_min_min = t_min_t1;
                    ///get normal of t3
                    Ray ray(source_ray_position, spot_lights[i]->light_dir);
                    struct point AB,AC;
                    struct point n;
                    AB.x = t3->b.x - t3->a.x;
                    AB.y = t3->b.y - t3->a.y;
                    ...
                    struct point temp;
                    temp = cross_prod(AB,AC);
                    //cout<<"cross=>"<<temp.x<<" "<<temp.y<<" "<<temp.z<<endl;


                    normal = unit_vector(temp);
                    //cout<<"unit normal=>"<<normal.x<<" "<<normal.y<<" "<<normal.z<<endl;
                    double d = dot_prod(negVector(ray.dir),normal);
                    ///double d = (ray.dir*(-1.0)).dot_product(normal);

                    if(d<=0){
                        normal = negVector(normal);
                    }


                    ///normal = t3->get_normal(Ray(source_ray_position, spotlights[i].direction));
                    normal = unit_vector(normal);
                    cout<<"unit normal in if=>"<<normal.x<<" "<<normal.y<<" "<<normal.z<<endl;
                }
                else {
                    //cout<<"in else"<<endl;
                    t_min_min = t_min_t2;
                    ...
                    ///normal = t3->get_normal(Ray(source_ray_position, spotlights[i].direction));
                    normal = unit_vector(normal);
                    cout<<"normal in else func_point: "<<normal.x<<", "<<normal.y<<", "<<normal.z<<endl;
                }

                if(t_min_min>=999999) return point(-999999,-999999,-999999);

                cout<<"t_min_min: "<<t_min_min<<endl;
                struct point intersection_point(source_ray_position.x + t_min_min*spot_lights[i]->light_dir.x, source_ray_position.y + t_min_min*spot_lights[i]->light_dir.y, source_ray_position.z + t_min_min*spot_lights[i]->light_dir.z);
                ///struct point intersection_point = Vector3D().getVector(source_ray_position, spotlights[i].direction, t_min_min);
                cout<<"intersection_point: "<<intersection_point.x<<", "<<intersection_point.y<<", "<<intersection_point.z<<endl;
                cout<<"normal outside func_point: "<<normal.x<<", "<<normal.y<<", "<<normal.z<<endl;
                struct point refracted_dir_1 = Refracted_dir(spot_lights[i]->light_dir, normal, (1/eta));
                cout<<refracted_dir_1.x<<endl;
                Ray second_ray(intersection_point, unit_vector(refracted_dir_1));


                /// 2nd face refraction

                t_min_t1 = t1->get_tMIN(&second_ray);
                t_min_t2 = t2->get_tMIN(&second_ray);

                cout<<"t_min_t1: "<<t_min_t1<<endl;
                cout<<"t_min_t2: "<<t_min_t2<<endl;
                ...
                if(t_min_t1<999999){
                    ...
                    normal = unit_vector(temp);
                    cout<<"unit normal=>"<<normal.x<<" "<<normal.y<<" "<<normal.z<<endl;
                    double d = dot_prod(negVector(second_ray.dir),normal);
                    ///double d = (ray.dir*(-1.0)).dot_product(normal);

                    if(d<=0){
                        normal = negVector(normal);
                    }
                    ///normal = t1->get_normal(second_ray);
                    normal = unit_vector(normal);
                    cout<<"unit normal2=>"<<normal.x<<" "<<normal.y<<" "<<normal.z<<endl;
                }
                else {
                    cout<<"in else"<<endl;
                    t_min_min = t_min_t2;
                    ...
                    ///normal = t2->get_normal(second_ray);
                    normal = unit_vector(normal);
                }

                if(t_min_min>=999999) return point(-999999,-999999,-999999);

                cout<<"t_min_min_2: "<<t_min_min<<endl;


                struct point intersection_point2(second_ray.start.x + t_min_min*second_ray.dir.x, second_ray.start.y + t_min_min*second_ray.dir.y, second_ray.start.z + t_min_min*second_ray.dir.z);
                cout<<"intersection_point2: "<<intersection_point2.x<<", "<<intersection_point2.y<<", "<<intersection_point2.z<<endl;

                struct point refracted_dir_2 = Refracted_dir(unit_vector(second_ray.dir), normal, (eta));
                cout<<"refracted_dir_2: "<<refracted_dir_2.x<<", "<<refracted_dir_2.y<<", "<<refracted_dir_2.z<<endl;
                Ray third_ray(intersection_point2, unit_vector(refracted_dir_2));

                cout<<third_ray.dir.x<<endl;

                /// floor position

                t_closest_object_min = 999999;
                closest_object_index = 999999;
                double t = 999999;
                double t_from_light_min = 999999;

                /// find the closest intersection point with the light source
                for(int j = 0; j < objects.size(); j++){
                    t = objects[j]->get_tMIN(&third_ray);
                    if(t>0 && t<t_from_light_min){
                        closest_object_index = j;
                        t_from_light_min = t;
                    }
                }
                ///return t_from_light_min;
                t_closest_object_min = t_from_light_min;
                ///t_closest_object_min = get_closest_object_t_min(third_ray, closest_object_index);

                if(closest_object_index != 999999){
                    cout<<"closest_object_index: "<<closest_object_index<<endl;
                    double *dc=new double[3];
                    double t_mini = objects[closest_object_index]->intersect(&third_ray, dc, 0);
                    cout<<"t_mini: "<<t_mini<<endl;

                    struct point intersection_point3(third_ray.start.x + t_mini*third_ray.dir.x, third_ray.start.y + t_mini*third_ray.dir.y, third_ray.start.z + t_mini*third_ray.dir.z);
                    cout<<"intersection_point3: "<<intersection_point3.x<<endl;
                    R_floor_points.push_back(intersection_point3);
                    return intersection_point3;
                }
                else{
                    cout<<"closest object index = 999999"<<endl;
                }
